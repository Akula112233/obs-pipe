...
show_help() {
    ...
    echo "  -s         Force redeploy SigNoz only"
    echo "  -v         Remove SigNoz volumes on redeploy (must be used with -f or -s)"
    echo "  -a         Force redeploy NextJS app only"
    echo
    echo "Examples:"
    echo "  $0                 # Deploy without forcing any redeployments"
    echo "  $0 -f             # Force redeploy everything"
    echo "  $0 -f -v          # Force redeploy everything and remove volumes"
    echo "  $0 -s -v          # Force redeploy SigNoz only and remove volumes"
...
# Validate flags
if [ "${clean_volumes}" = true ] && [ "${force_signoz}" != true ] && [ "${force_all}" != true ]; then
    echo "Error: -v must be used with -f or -s"
    exit 1
fi
...
SUBDOMAIN="${NEXT_PUBLIC_ORG_ID}.app.trysift.dev"  # Get NEXT_PUBLIC_ORG_ID from env vars
...
force_signoz="${FORCE_SIGNOZ:-false}"
...
signoz_running=false
if cd /app/signoz/deploy/docker && sudo ${docker_compose_cmd} ps -q | grep -q .; then
    if curl -s -o /dev/null -w '%{http_code}' 'http://localhost:3301/api/v1/health?live=1' | grep -q '200'; then
        echo 'SigNoz is already running'
        signoz_running=true
    fi
fi

# Handle SigNoz deployment based on force flag and current status
if [ "${signoz_running}" = true ] && [ "${force_signoz}" != "true" ]; then
    echo 'Using existing SigNoz deployment'
else
    # Check if we need to clean up
    need_cleanup=false
    if [ "${force_signoz}" = "true" ]; then
        need_cleanup=true
        echo 'Force flag set, will clean up existing SigNoz deployment'
    elif [ "${signoz_running}" = false ] && [ -n "$(sudo docker ps -aq -f name=signoz-)" ]; then
        need_cleanup=true
        echo 'Found partially deployed SigNoz containers, will clean up before deploying'
    fi
    ...
    if [ "${need_cleanup}" = true ]; then
        ...
        # Stop and remove all SigNoz and related containers
        echo 'Stopping and removing SigNoz and related containers...'
        # First try docker-compose down with or without volume removal
        if [ "${clean_volumes}" = true ]; then
            echo 'Removing containers and volumes...'
            COMPOSE_HTTP_TIMEOUT=200 sudo ${docker_compose_cmd} down -v || true
        else
            echo 'Removing containers only (keeping volumes)...'
            COMPOSE_HTTP_TIMEOUT=200 sudo ${docker_compose_cmd} down || true
        fi
        
        # Then forcefully remove any remaining containers
        # Include both signoz- prefixed and known SigNoz container names
        sudo docker ps -aq -f name=signoz- -f name=schema-migrator -f name=clickhouse -f name=zookeeper -f name=otel-collector -f name=alertmanager -f name=query-service -f name=frontend | xargs -r sudo docker rm -f
        
        # Remove volumes only if clean_volumes is true
        if [ "${clean_volumes}" = true ]; then
            echo 'Removing SigNoz volumes...'
            sudo docker volume ls -q -f name=signoz- | xargs -r sudo docker volume rm
        fi
        
        # Remove network
        echo 'Removing Docker networks...'
        sudo docker network rm signoz-net || true
        
        # Additional cleanup for any stuck containers by exact name
        for container in init-clickhouse zookeeper-1 clickhouse otel-collector alertmanager query-service frontend schema-migrator-sync schema-migrator-async; do
            sudo docker rm -f "signoz-${container}" "${container}" 2>/dev/null || true
        done

        # Ensure clean state before creating network
        sleep 5
    fi
    
    # Create required networks if they don't exist
    if ! sudo docker network inspect signoz-net >/dev/null 2>&1; then
        echo 'Creating Docker networks...'
        sudo docker network create \
            --label com.docker.compose.network=signoz-net \
            --label com.docker.compose.project=signoz \
            signoz-net
    fi
    ...
    cd /app/signoz/deploy/docker
    COMPOSE_HTTP_TIMEOUT=200 sudo ${docker_compose_cmd} pull
    
    # Start with a fresh state if we cleaned up
    if [ "${need_cleanup}" = true ]; then
        COMPOSE_HTTP_TIMEOUT=200 sudo ${docker_compose_cmd} down --remove-orphans || true
    fi
    
    COMPOSE_HTTP_TIMEOUT=200 sudo ${docker_compose_cmd} up -d --force-recreate --remove-orphans
    
    # Wait for SigNoz services to be ready
    echo 'Waiting for SigNoz services to start...'
    sleep 60
fi
...
if [ "${app_running}" = true ] && [ "${force_app}" != "true" ]; then
    ...
    # Wait longer for services to start due to build time
    ...
    if [ ${retry_count} -eq ${max_retries} ]; then
        ...
        echo 'For more help: https://signoz.io/docs/install/troubleshooting/'
        echo '++++++++++++++++++++++++++++++++++++++++'
...
# Wait for SigNoz health endpoint
signoz_retries=30
signoz_retry_count=0
while [ ${signoz_retry_count} -lt ${signoz_retries} ]; do
    if curl -s -o /dev/null -w '%{http_code}' 'http://localhost:3301/api/v1/health?live=1' | grep -q '200'; then
        echo 'SigNoz services started successfully'
        break
    fi
    echo "Waiting for SigNoz services... Attempt $((signoz_retry_count + 1))/${signoz_retries}"
    signoz_retry_count=$((signoz_retry_count + 1))
    sleep 10
done

if [ ${signoz_retry_count} -eq ${signoz_retries} ]; then
    echo '+++++++++++ ERROR ++++++++++++++++++++++'
    echo 'SigNoz services failed to start. Please check:'
    echo '1. Docker logs below'
    echo '2. Ports 3301 and 4317 are available'
    echo '3. System has enough resources'
    echo '4. Try bringing down all containers with:'
    echo "   cd /app/signoz/deploy/docker && sudo ${docker_compose_cmd} down -v"
    echo 'For more help: https://signoz.io/docs/install/troubleshooting/'
    echo '++++++++++++++++++++++++++++++++++++++++'
    sudo ${docker_compose_cmd} logs
    exit 1
...
EOF
    ...
    gcloud compute ssh $COMPUTE_ENGINE_SERVICE_NAME --zone=$GCP_ZONE -- "chmod +x ~/deploy/deploy.sh && FORCE_SIGNOZ=${force_signoz} FORCE_APP=${force_app} CLEAN_VOLUMES=${clean_volumes} ~/deploy/deploy.sh" || {
